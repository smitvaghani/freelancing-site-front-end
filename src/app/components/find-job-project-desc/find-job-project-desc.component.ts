...
export class FindJobProjectDescComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bidForm = new UntypedFormGroup({
      bidAmountControl: new UntypedFormControl("", [Validators.required, Validators.min(0)]),
      daysControl: new UntypedFormControl("", [Validators.required, Validators.min(1)]),
      proposalControl: new UntypedFormControl("", Validators.required)
    })
    this.workForm = new UntypedFormGroup({
